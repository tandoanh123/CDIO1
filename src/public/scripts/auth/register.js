...
function validateInput() {
  ...
  if (
    ...
    !isPasswordValid(passwordConfirmValue)
  ) {
    setError(
      password,
      "Mật khẩu phải ít nhất 8 ký tự, trong đó có cả chữ và số, không bao gồm dấu cách và ký tự đặc biệt!"
    );
    setError(
      passwordConfirm,
      "Mật khẩu phải ít nhất 8 ký tự, trong đó có cả chữ và số, không bao gồm dấu cách và ký tự đặc biệt!"
    ...
    document
      .getElementById("form__password-confirm")
  ...
  if (isAllValid) {
    ...
    console.log("Sending register data:", register);
    ...
    fetch("/auth/register", {
      ...
      .then((back) => {
        console.log("Server response:", back);
        if (back.status === "error") {
          ...
          console.error("Unexpected status:", back.status);
          alert("Phản hồi không mong đợi từ server!");
        }
      })
      .catch(function (err) {
